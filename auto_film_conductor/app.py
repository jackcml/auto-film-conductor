...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def current_round(service: Annotated[ConductorService, Depends(conductor)]):
        return await _call(service.current_round)

    @app.post("/rounds/start", response_model=Round)
    async def start_round(service: Annotated[ConductorService, Depends(conductor)]):
    ...
    async def suggest(
        ...
        service: Annotated[ConductorService, Depends(conductor)],
    ):
    ...
    async def override(round_id: int, request: OverrideRequest, service: Annotated[ConductorService, Depends(conductor)]):
    ...
    async def suggestions(round_id: int, session: Annotated[Session, Depends(get_session)]):
    ...
    async def polls(round_id: int, session: Annotated[Session, Depends(get_session)]):
        return session.exec(select(PollRecord).where(PollRecord.round_id == round_id)).all()

    @app.post("/playback/stop")
    async def stop_playback(service: Annotated[ConductorService, Depends(conductor)]):
        await _call(service.stop_playback)
        return {"ok": True}

    @app.post("/mock-polls")
    async def create_mock_poll(request: MockPollCreateRequest, provider: Annotated[MockVotingProvider, Depends(voting)]):
        poll_id = await _call(
    ...
    async def vote_mock_poll(
        ...
        provider: Annotated[MockVotingProvider, Depends(voting)],
    ):
    ...
    async def close_mock_poll(poll_id: str, provider: Annotated[MockVotingProvider, Depends(voting)]):
        await _call(provider.close_poll, poll_id)
        return {"ok": True}

    @app.get("/mock-polls/{poll_id}/results")
    async def mock_poll_results(poll_id: str, provider: Annotated[MockVotingProvider, Depends(voting)]):
